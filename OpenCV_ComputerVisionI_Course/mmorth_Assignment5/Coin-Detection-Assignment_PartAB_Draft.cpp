...
drawContours(coinCircles, contours, 1, Scalar(0,0,255), 20);
Point2f center;
float radius;
for (size_t i=0; i < contours.size(); i++){
    if (i != 2 && i != maxContourIndex) {
        ...
        circle(coinCircles,center,radius, Scalar(125,125,125), 2);
    }
}
...
drawContours(coinCircles2, contours, 1, Scalar(0,0,255), 20);
Point2f center2;
float radius2;
for (size_t i=0; i < contours.size(); i++){
    if (i != maxContourIndex && i != 5 && i != 6) {
        ...
        circle(coinCircles2,center2,radius2, Scalar(125,125,125), 2);
    }
}
