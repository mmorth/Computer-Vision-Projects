...
int main()
{
    ...
    Mat frame = imread("C:\\Users\\mmort\\GIT\\CprE575\\Homework\\Homework3\\HW3_2018\\HW3\\input\\pacman_templates\\pacmanLivesFrame.png");

//    // Create the masks for the ghosts and Pacman
//    Mat pacmanMask;
//    inRange(frame, Scalar(0, 200, 200), Scalar(210, 255, 255), pacmanMask);
//    imshow("pacmanMask", pacmanMask);

//    Mat redGhostMask;
//    inRange(frame, Scalar(0, 0, 150), Scalar(50, 50, 255), redGhostMask);
//    imshow("redGhostMask", redGhostMask);

//    Mat pinkGhostMask;
//    inRange(frame, Scalar(70, 170, 220), Scalar(255, 255, 255), pinkGhostMask);
//    imshow("pinkGhostMask", pinkGhostMask);

//    Mat lightBlueGhostMask;
//    inRange(frame, Scalar(210, 170, 0), Scalar(255, 255, 90), lightBlueGhostMask);
//    imshow("lightBlueGhostMask", lightBlueGhostMask);

//    Mat orangeGhostMask;
//    inRange(frame, Scalar(60, 160, 210), Scalar(120, 200, 255), orangeGhostMask);
//    imshow("orangeGhostMask", orangeGhostMask);
...
void processVideo(VideoCapture cap, string inputVideoName, string outputVideoName) {
    ...
    // TODO: Determine how you are going to handle the situation where the ghosts' colors change when the eating pellet is taken
        // Could find the one that is closest to the last known position
            // Create a mask for the blinking ghosts (one for on and one or off)
            // For each ghost, determine which blinking ghost is closest to the last known position (within certain threshold)
            // Draw the path to that closest ghost
    ...
    ghost redGhost;
    redGhost.lowRangeColor = Scalar(0, 200, 200);
    redGhost.highRangeColor = Scalar(210, 255, 255);
    redGhost.minArea = 50;
    Vec3b redColor;
    redColor.val[0] = 25;
    redColor.val[1] = 25;
    ...
    ghost pinkGhost;
    pinkGhost.lowRangeColor = Scalar(70, 170, 220);
    pinkGhost.highRangeColor = Scalar(255, 255, 255);
...
void markPacmanPath(Mat frame, Mat mask, Mat path, ghost curGhost) {
    ...
    for (int i = 0; i < contours.size(); i++) {
        if (contourArea(contours[i]) > curGhost.minArea) {
